...
COPY . /app/.
RUN --mount=type=cache,id=wYoWEFsbbc-/root/npm,target=/root/.npm npm ci

# build phase
COPY . /app/.
RUN --mount=type=cache,id=wYoWEFsbbc-next/cache,target=/app/.next/cache --mount=type=cache,id=wYoWEFsbbc-node_modules/cache,target=/app/node_modules/.cache npm run build
