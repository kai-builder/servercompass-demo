...
COPY . /app/.
RUN --mount=type=cache,id=4yzfLdcDnoM-/root/npm,target=/root/.npm npm ci

# build phase
COPY . /app/.
RUN --mount=type=cache,id=4yzfLdcDnoM-next/cache,target=/app/.next/cache --mount=type=cache,id=4yzfLdcDnoM-node_modules/cache,target=/app/node_modules/.cache npm run build
